...
def db_connection():
    connection = pymssql.connect(server=MSSQL_HOST, user=MSSQL_USER, password=MSSQL_PASSWORD, database=MSSQL_DATABASE, port=MSSQL_PORT)
    try:
        yield connection
    finally:
        connection.close()
...
def test_duplicates(db_connection):
    cursor = db_connection.cursor()
    cursor.execute("WITH cte AS (SELECT job_id, count(*) as cnt FROM hr.jobs "
                                           "GROUP BY job_id HAVING count(*)>1) "
                                           "SELECT CASE WHEN count(*)=0 THEN 1 ELSE 0 END AS res FROM cte")
    result = cursor.fetchone()
...
def test_min_max_salary_comparison(db_connection):
    cursor = db_connection.cursor()
    cursor.execute("WITH cte AS (SELECT * FROM hr.jobs WHERE min_salary>max_salary) "
                                           "SELECT CASE WHEN count(*)=0 THEN 1 ELSE 0 END AS res FROM cte")
    result = cursor.fetchone()
...
def test_email(db_connection):
    cursor = db_connection.cursor()
    cursor.execute("with cte as (SELECT * FROM hr.employees WHERE email NOT LIKE '%__@__%.__%') "
                                           "SELECT CASE WHEN count(*)=0 THEN 1 ELSE 0 END AS res FROM cte")
    result = cursor.fetchone()
...
def test_main_manager_salary(db_connection):
    cursor = db_connection.cursor()
    cursor.execute("WITH cte AS (SELECT * FROM hr.employees "
                                           "WHERE manager_id is null and salary = "
                                           "(SELECT MAX(salary) FROM hr.employees)) "
                                           "SELECT CASE WHEN COUNT(*)=0 THEN 0 ELSE 1 END AS res FROM cte")
    result = cursor.fetchone()
